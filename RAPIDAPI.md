...
- Endpoint chamado: `GET /v1/audio-features/{trackId}`
...
**Uma requisição por análise.** Título, artista e capa vêm do MusicBrainz/iTunes,
então o endpoint `/v1/tracks` não é chamado (o tipo `SpotifyTrackMeta` existe mas
não é usado para fetch).

## Cache

`Map` em memória no processo do servidor, indexado por `trackId`, guardando
**só sucessos**. A mesma faixa gasta cota **uma vez**; repetições saem do cache.
Não é persistido — reiniciar o servidor limpa.
...
1. **Prompt** — vira um bloco `DADOS REAIS DA FAIXA` (energia/humor/dançabilidade/
   acústico/instrumental/tom/duração em faixas textuais) que orienta o GPT a
   alinhar gênero, voz e instrumentos ao som real.
2. **BPM** — `Math.round(tempo)` substitui o `bpm` estimado.
3. **Estrutura** — `structureFromDuration(durationMs)` substitui o palpite:
   `> 5 min` → `estendida`, `>= 3 min` → `completa`, senão `padrao`.
4. **Resposta** — `audio: features` e `audioError: null`.
...
- **Compositor › Inspire-se** — `InspireBox` extrai o trackId do link colado e
  manda em `spotifyTrackId` para `/api/inspire`.
